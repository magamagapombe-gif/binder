...
export default function SwipeCard({ profile, onSwipe, isTop }: Props) {
  ...
  return (
    <motion.div style={{ x, rotate, position: 'absolute', width: '100%', touchAction: 'none' }}
      ...
      onDragStart={(_, info) => { dragStart.current = { x: info.point.x, y: info.point.y }; }}
      ...
      <motion.div style={{ opacity: passOpacity }}
        ...
        <span className="text-red-400 font-black text-3xl">PASS</span>
      </motion.div>
      ...
      <div className="relative w-full aspect-[3/4] rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div className="absolute top-3 inset-x-3 z-10 flex gap-1">
          {photos.map((_: string, i: number) => (
            <div key={i} className={`h-0.5 flex-1 rounded-full ${i === photoIdx ? 'bg-white' : 'bg-white/30'}`} />
          ))}
        </div>

        <img src={photos[photoIdx]} alt={profile.name} className="w-full h-full object-cover" draggable={false} />
          ...
          <div className="flex items-end justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                {profile.users?.is_verified && <Shield size={16} className="text-[#FF4458]" />}
              </div>
              {profile.bio && <p className="text-white/70 text-sm mt-1 line-clamp-2">{profile.bio}</p>}
              <div className="flex items-center gap-1 mt-2 text-white/40 text-xs">
                <MapPin size={12} />
                <span>{profile.country === 'UG' ? 'Uganda' : profile.country === 'KE' ? 'Kenya' : 'Tanzania'}</span>
              </div>
            </div>
          </div>
        </div>
